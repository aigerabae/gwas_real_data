...
```bash
awk -F'\t' '{for(i=57;i<=101;i++) if($i != ".") {print $0; next}; for(i=22;i<=26;i++) if($i != ".") {print $0; next}}' kaz_gwas_for_annovar_224.FINAL.annovar.hg38_multianno.header.txt > rows_with_mafs.tsv
```
